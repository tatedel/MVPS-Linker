...
function findVexStubPath() {
    const home = os.homedir();
    const platform = process.platform;
    const globalStorageBase = platform === "win32"
...
function activate(context) {
    ...
    const templateCmd = vscode.commands.registerCommand("mvps-linker.createTemplate", async () => {
        ...
        fs.writeFileSync(path.join(vscodeDir, "extensions.json"), JSON.stringify({ recommendations: ["ms-python.python"] }, null, "\t"));
        const settings = {
        ...
        fs.writeFileSync(path.join(vscodeDir, "settings.json"), JSON.stringify(settings, null, "\t"));
        const now = new Date().toLocaleString("en-US");
        fs.writeFileSync(path.join(vscodeDir, "vex_project_settings.json"), JSON.stringify({
        ...
        vscode.commands.executeCommand("vscode.openFolder", vscode.Uri.file(root), { forceNewWindow: false }).then(() => {
